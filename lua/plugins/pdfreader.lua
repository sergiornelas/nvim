-- This plugin is fully inert unless working on **/CV/*_CV.yaml files
...
vim.api.nvim_create_autocmd({ "BufReadPost", "BufNewFile" }, {
	pattern = "**/CV/*_CV.yaml",
	callback = function()
		require("lazy").load({ plugins = { "pdfreader.nvim" } })
	end,
})
...
return {
	...
	config = function()
		...
		local function is_valid_cv_yaml(path)
			return path:match("CV/[^/]+_CV%.yaml$")
		end
		...
		local function build_pdf_paths(yaml_file_path)
			local pdf_name = get_pdf_name(yaml_file_path)
			local cv_dir = vim.fn.fnamemodify(yaml_file_path, ":h")
			return pdf_name, cv_dir, cv_dir .. "/" .. pdf_name
		end

		local function with_debounce(ms, fn)
			...
			render_timer:start(ms, 0, vim.schedule_wrap(fn))
		end

		local function render_cv(yaml_file_path)
			...
			local pdf_name, _, expected_pdf_path = build_pdf_paths(yaml_file_path)
		...
		vim.api.nvim_create_autocmd("BufWritePost", {
			...
			callback = function(args)
				...
				with_debounce(DEBOUNCE_MS, function()
					render_cv(yaml_file_path)
				end)
			end,
		})
		...
		vim.api.nvim_create_autocmd("VimResized", {
			...
			callback = function(args)
				...
				if not is_valid_cv_yaml(yaml_file_path) then
					return
				end

				with_debounce(DEBOUNCE_VIM_RESIZE_MS, function()
					local _, _, expected_pdf_path = build_pdf_paths(yaml_file_path)
					reload_pdf(expected_pdf_path)
				end)
			end,
		})
